...
func Init() {
	...
	// Creates a client. TODO make this SQL?
	...
	fs := http.FileServer(http.Dir(exPath + string(os.PathSeparator) + "dist"))

	mux.PathPrefix("/dist/").Handler(http.StripPrefix("/dist/", fs))

	fs = http.FileServer(http.Dir(exPath + string(os.PathSeparator) + "assets"))
	...
	genericHandle = func(w http.ResponseWriter, r *http.Request) {
		http.ServeFile(w, r, exPath+string(os.PathSeparator)+"/index.html")
	}
